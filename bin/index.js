...
pg.init=function(size,options){__pg.init(size,util.unescape(util.encodeFormData(options," ")),function(error){console.error(error)})};pg.exec=function(query,complete,cancel){__pg.exec(query,function(error,result){if(error.length>0)cancel(error);else complete(result)})};pg.execPrepared=function(query,params,complete,cancel){pg.exec(pg.prepareQuery(query,params),complete,cancel)};pg.destroy=function(){__pg.destroy()};
module.exports = pg;
