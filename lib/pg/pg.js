 ...
 * @type {number}
 */
pg.__criticalExecutionTime = 5000;
...
pg.init = function(size, options, opt_errorHandler, opt_stackDeepWarnHandler,
  ...
  if (opt_logger) {
    pg.logger = opt_logger;
    pg.__criticalExecutionTime = opt_criticalExecutionTime ?
        opt_criticalExecutionTime : 5000;
  }
...
pg.exec = function(query, complete, cancel) {
  ...
  var startExecutionTime = Date.now();
  pg.logger('pg_used_connections:' + pg.__requestCounter++);

  __pg.exec(query, function(error, result) {
    pg.__requestCounter--;
    var executionTime = Date.now() - startExecutionTime;
    if (executionTime > pg.__criticalExecutionTime) {
      pg.logger('pg_critical_execution:' + executionTime + ', query: ' + query);
    }

    if (error.length > 0) {
      pg.__errorTasks++;
      pg.logger('pg_error_tasks:' + pg.__errorTasks);
      cancel(error);
    } else {
      complete(result);
    }
