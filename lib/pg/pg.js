 ...
 * @param {string=} opt_actionWorkerId
 */
pg.init = function(size, options, opt_errorHandler, opt_actionWorkerId) {
  /**
   * @type {string}
   */
  pg.__actionWorker = opt_actionWorkerId ? opt_actionWorkerId : '';
...
pg.exec = function(query, complete, cancel) {
  console.warn('action_worker_' + pg.__actionWorker +
      ',pg_used_connections:' + pg.__requestCounter++);
  __pg.exec(query, function(error, result) {
    console.warn('action_worker_' + pg.__actionWorker +
        ',pg_used_connections:' + pg.__requestCounter--);
    if (error.length > 0) {
      pg.__errorConnections++;
      console.warn('action_worker_' + pg.__actionWorker +
          ',pg_error_connections:' + pg.__errorConnections);
      cancel(error);
    } else {
      complete(result);
    }
